...
public class NonNativeKeyboard : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        var canvasBounds = RectTransformUtility.CalculateRelativeRectTransformBounds(transform);

        var rect = GetComponent<RectTransform>();
        m_ObjectBounds = new Vector3(canvasBounds.size.x * rect.localScale.x, canvasBounds.size.y * rect.localScale.y,
            canvasBounds.size.z * rect.localScale.z);
    ...
    public void Backspace()
    {
        ...
        if (InputField.selectionFocusPosition != InputField.caretPosition ||
            InputField.selectionAnchorPosition != InputField.caretPosition)
        {
            ...
            {
                InputField.text = InputField.text[..InputField.selectionFocusPosition] +
                                  InputField.text[InputField.selectionAnchorPosition..];
                InputField.caretPosition = InputField.selectionFocusPosition;
            }
            else // left to right
            {
                InputField.text = InputField.text[..InputField.selectionAnchorPosition] +
                                  InputField.text[InputField.selectionFocusPosition..];
